...
1. On the **Appliance Configuration Manager** page, verify that the **Check connectivity to Azure** and **Check time is in sync with Azure** steps successfully completed. 
    ...
    > **Note:** You might run into an issue with the step **Check connectivity to Azure** failing. If so, ignore it for the time being. 
...
1. In the section **2. Download Azure Migrate appliance**, note the **.VHD file. 12 GB** option next to the **Download** button. 
    ...
    > **Note:** The VHD file has been already downloaded to the lab computer and set up to expedite the process of provisioning the appliance. Do **NOT** initiate the download since this would take an extensive amount of time. 
...
1. Within the Virtual Machine Connection window, in the web browser displaying the **Appliance Configuration Manager** page, verify that the **Check connectivity to Azure** and **Check time is in sync with Azure** steps successfully completed. 
    ...
    > **Note:** If you are still seeing the **Check connectivity to Azure** step failing, keep refreshing the browser page until this step completes successfully.

1. In the second **Check latest updates and register appliance** section, in the **Register Hyper-V appliance by passing the key here** text box, enter the project key you recorded earlier in this task and select **Verify**.
